...
create_socat_instance() {
  ...
  screen -mdS "socat-$outbound_port-$outbound_host" && screen -S "socat-$outbound_port-$outbound_host" -p 0 -X stuff "socat -d -d -lf /var/log/socat.log TCP-LISTEN:$inbound_port,fork,range=$RANGE TCP-CONNECT:$outbound_host:$outbound_port^M"
...
nysm_install() {
  ...
  apt-get install -t jessie-backports -y inetutils-ping net-tools screen dnsutils curl openssl openjdk-8-jre-headless
  check_errors

  nysm_action "Installing nginx, socat, certbot..."
  apt-get install -t jessie-backports -y socat nginx python-certbot-nginx
  check_errors
